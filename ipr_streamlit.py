...
with col1:
    pr = st.slider("Pr (psi)", min_value=500, max_value=6000, value=2200, step=1)
    pb = st.slider("Pb (psi)", min_value=500, max_value=6000, value=2000, step=1)

with col2:
    q_test = st.slider("Q_test (BPD)", min_value=50, max_value=10000, value=500, step=1)
    pwf_test = st.slider("Pwf_test (psi)", min_value=0, max_value=6000, value=1500, step=1)

mostrar_escenarios = st.checkbox("Mostrar escenarios -5% y -10% en Pwf_test", value=False)

if pwf_test >= pr:
    st.error(f"No se permite Pwf_test mayor o igual que Pr ({pr} psi). Valor ingresado: Pwf_test={pwf_test}")
    st.stop()
